...
from engine.flights import get_flight_data

# Group Consensus Feature Routers
from routers.auth_router import router as auth_router
from routers.groups_router import router as groups_router
from routers.admin_groups_router import router as admin_groups_router
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        print(f"[DB] PostgreSQL init skipped (not available): {exc}")

    # Create group feature tables (idempotent)
    try:
        from database import init_models
        await init_models()
        print("[DB] Group feature tables initialised")
    except Exception as exc:
        print(f"[DB] Group table init skipped: {exc}")

    # ── Background ML Precomputations ──────────────────────────────────────
    # We do NOT await these here to avoid boot-time memory spikes and timeouts.
    try:
        from engine.data_loader import load_data, get_destinations
        load_data()
        df = get_destinations()
        if df is not None:
            from engine.semantic_search import precompute_embeddings
            from engine.clip_engine import precompute_clip_embeddings
            
            # Background tasks - these will load models lazily when they run
            asyncio.create_task(precompute_embeddings(df))
            asyncio.create_task(precompute_clip_embeddings(df))
            print("[Startup] ML precomputation tasks backgrounded (Lazy & Non-blocking).")
    except Exception as exc:
        print(f"[Startup] Precomputation backgrounding failed: {exc}")

    yield
    # Graceful shutdown: close Redis pool
    try:
        from engine.redis_cache import close_redis
        await close_redis()
    except Exception:
        pass
...
app.add_middleware(
    CORSMiddleware,
    # Explicit origin list — add your Vercel URL once you know it.
    ...
    allow_origins=[
        ...
        "https://allora.vercel.app",
        "https://allora-tourism.vercel.app",
        # ── Hugging Face Spaces Backend ────────────────────────────
        # Replace <username> with your HF username:
        "https://srid0308-allora.hf.space",
        # Wildcard for Vercel preview deployments (branch/PR previews):
        # Note: Vercel automatically handles CORS for preview URLs if needed,
        # but listing the production URL explicitly is sufficient for most cases.
    ],
...
# Register Group Consensus Feature routers
...
async def recommend(req: RecommendRequest):
    ...
    history_ids = await get_user_history(req.user_id, limit=20)
    ...
    # ── Timing: start enrichment ──
    t_enrich = time.perf_counter()

    # Only enrich the top 10 candidates (LLM picks 5; sending 30 wastes API calls)
    import asyncio as _aio
    _flight_sem = asyncio.Semaphore(5)  # max 5 concurrent flight API calls

    candidates_to_enrich = candidates[:10]
    ...
    async def _enrich_candidate(c):
        ...
        async with _flight_sem:
            dest_dict["flight_data"] = await get_flight_data(req.origin, dest_iata)
        dest_dict["user_comments"] = await get_social_context(str(c["id"]))
        return dest_dict
        
    raw_data_for_llm_tuples = await _aio.gather(*[_enrich_candidate(c) for c in candidates_to_enrich])
    raw_data_for_llm = list(raw_data_for_llm_tuples)
    print(f"[Timing] Candidate enrichment: {round((time.perf_counter() - t_enrich) * 1000)}ms")
    ...
    db_liked_cats = await get_liked_categories(req.user_id)
    ...
    if ai_response_list is None:
        ...
        print(f"[Timing] LLM Agent call: {round((time.perf_counter() - t_llm) * 1000)}ms")
        if ai_response_list and not req.surprise_mode:
            recommendation_cache.set(cache_key, ai_response_list)
    else:
        print(f"[Cache] HIT for key {cache_key[:8]}… (0ms)")
    ...
    for pick in ai_response_list:
        ...
        if pid in candidates_by_id:
            ...
            dest["is_surprise"] = bool(req.surprise_mode)
    ...
    # ── RAG + Photos: run both enrichments concurrently to save time ──
    ...
    # Run Wikidata RAG and photo enrichment concurrently
    async def _post_enrich():
        results = await _aio.gather(*[_enrich_xai(r) for r in final_results])
        return list(results)

    async def _photo_enrich(results_list):
        if req.include_photos:
            return await enrich_destinations_with_photos(results_list)
        return results_list

    final_results = await _post_enrich()
    final_results = await _photo_enrich(final_results)
    print(f"[Timing] Post-enrichment (RAG + photos): {round((time.perf_counter() - t_post) * 1000)}ms")

    # 5. Log the new recommendations back to PostgreSQL history
    shown_ids = [str(r["id"]) for r in final_results]
    await add_to_history(req.user_id, shown_ids)
    ...
    return {
        ...
        "user_id": req.user_id,
        "surprise_mode": bool(req.surprise_mode),
        "novelty_count": int(sum(1 for r in final_results if r.get("is_novelty"))),
        "discovery_count": int(sum(1 for r in final_results if r.get("is_discovery"))),
        "weather_flagged_count": int(sum(1 for r in final_results if r.get("weather_flagged"))),
        "pivot_count": int(sum(1 for r in final_results if r.get("pivot_applied"))),
...
async def like_destination(req: LikeRequest):
    ...
    already_liked = await is_liked(req.user_id, req.destination_id)
    if already_liked:
        await unlike(req.user_id, req.destination_id)
        return {"status": "unliked", "user_id": req.user_id, "destination_id": req.destination_id, "liked": False}
    else:
        await save_like(req.user_id, req.destination_id, categories)
        return {
            "status": "liked",
            "user_id": req.user_id,
...
async def get_user_likes(user_id: str):
    ...
    """
    liked_ids = await get_liked_destination_ids(user_id)
    liked_cats = await get_liked_categories(user_id)
...
async def semantic_search_endpoint(
    ...
    results = await semantic_search(q, top_n=min(top_n, 30))
    if not results:
        return {
            "query": q,
            "total": 0,
...
@app.post("/search/visual", tags=["Visual Search"])
async def visual_search_endpoint(
    file: UploadFile = File(...),
    top_n: int = 10,
):
    """
    Upload an image to find destinations with a similar 'vibe'.
    Uses CLIP embeddings + pgvector cosine similarity.
    ...
    try:
        from engine.clip_engine import encode_image
        query_vector = encode_image(image_bytes)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"CLIP encoding failed: {str(e)}")

    # Query pgvector for closest CLIP embeddings
    ...
    async with async_session() as session:
        stmt = (
            select(
                Destination,
                Destination.clip_embedding.cosine_distance(query_vector).label("distance"),
            )
            .where(Destination.clip_embedding.isnot(None))
            .order_by("distance")
            .limit(min(top_n, 30))
        )
    ...
    for dest, distance in rows:
        results.append({
            ...
            "visual_similarity": round(1.0 - float(distance), 4),
        })
